<template>
  <div class="p-4 sm:p-6 bg-gray-100 min-h-screen sm:pl-[260px]">
    ...
    <div class="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
      <div></div>
      <h2 class="text-2xl sm:text-3xl font-bold text-gray-800 my-5">Home Appliances</h2>
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      <div v-for="item in appliances" :key="item.id"
        ...
        <div class="relative">
          <img
            :src="item.image || 'https://www.shutterstock.com/shutterstock/photos/1396219259/display_1500/stock-vector-home-appliances-vector-drawings-set-cartoon-illustration-of-washing-machine-refrigerator-tv-1396219259.jpg'"
            alt="Home Appliance" class="w-full h-40 sm:h-48 object-cover rounded-lg mb-3" />
        </div>
...
export default {
  ...
  methods: {
    async addToCart(item) {
      ...
      console.log(item);
      // alert(`${item.appliance} added to cart!`);
      Swal.fire({
        position: "center",
        icon: "success",
        title: "Item Added to Cart Successfully!",
    ...
    async getAllAppliances() {
      this.isLoading = true;
      try {
        const response = await axios.get("https://ecommerce-hjlr.onrender.com/home/getall", {
          headers: {
            Authorization: 'Bearer ' + this.userObj.token
          }
        }); // Your Appliances API
  ...
  mounted() {
    ...
    // this.userObj = localStorage.getItem('user')=='string'?JSON.parse(localStorage.getItem('user')):localStorage.getItem('user');
